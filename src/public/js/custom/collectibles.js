jQuery(document).ready(function() {
  var table = $("#collectible_list").DataTable({
    ...
    ajax: {
      ...
      type: "POST",
      data: function (d) {
          (d.search_collectible = $("#search_collectible").val()),
          (d.token_type = $("#token_type").val()),
          (d.search_collectible_by_category = $("#search_collectible_by_category").val()),
          (d.search_collectible_by_artist = $("#search_collectible_by_artist").val()),
          (d.collectible_ban_status = $("#collectible_status").val()),
          (d.collectible_hide_status = $("#filter_is_hide").val()),
          (d.collectible_featured_status = $("#filter_is_featured").val()),
          (d.collectible_burn_status = $("#filter_is_burn").val());
      },
    },
    columnDefs: [
  ...
  var table_collectible_owners = $("#collectible_owner_list").DataTable({
    ...
    ajax: {
      ...
      type: "POST",
      data: function (d) {
        (d.collectible_uuid = $("#collectible_uuid").val()),
          (d.token_type = $("#token_type").val()),
          (d.search_collectible_by_category = $(
            "#search_collectible_by_category"
          ).val()),
          (d.search_collectible_by_artist = $(
            "#search_collectible_by_artist"
          ).val()),
          (d.search_collectible_by_owner = $(
            "#search_collectible_by_owner"
          ).val());
      },
    },
    columnDefs: [
  ...
  var collections = $("#collectiion_list").DataTable({
    ...
    ajax: {
      ...
      type: "POST",
      data: function (d) {
        (d.collection_name = $("#collection_name").val()),
          (d.token_type = $("#token_type").val()),
          (d.collection_type = $("#collection_type").val());
      },
    },
  ...
  var collectible_curated_list = $("#collectible_curated_list").DataTable({
    ...
    ajax: {
      ...
      data: function (d) {
          ...
          d.search_collectible_by_category = $("#search_collectible_by_category").val(),
          d.search_collectible_by_artist = $("#search_collectible_by_artist").val(),
          d.search_collectible_by_owner = $("#search_collectible_by_owner").val();
      },
    },
    columnDefs: [
      { orderable: false, targets: 0 },
  ...
  var validator;
	...
	var _initDemo1 = function () {
		...
		validator = FormValidation.formValidation(
			document.getElementById('editCollectible'),
			{
				fields: {
