...
module.exports.getUserDetailsUsingPublicAddress = async function (userPublicAddress) {
  return User
        .findOne({ where: { user_public_address: userPublicAddress } })
        .then(function(obj) {
            if(obj) {
                 // update
  ...
  return Collections
        .findOne({ where: { tokenAddress: tokenAddress } })
        .then(function(obj) {
            if(obj) {
                 // update
...
module.exports.addNewUserRecord = function (userPublicAddress) {
  ...
  module.exports.getUserNonceGenerated = async function (userPublicAddress, userAction) {
    ...
    return Nonce
        .findOne({ where: { user_public_address: userPublicAddress, 'is_used': false } })
        .then(function(obj) {
            if(obj) {
                 // update
  ...
  return async (req, res, next) => {
    try {
      ...
      if (signedBy.toLowerCase() == userPublicAddress) {
        logger.info(
          "signature verification successful. proceed Further using next()"
        );
