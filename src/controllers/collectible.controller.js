            ...
            collectibles.forEach(  (element) => {
              let actions = ``;
                ...
                formatted_result.push([
                                    ...
                                    element.collectible_category,
                                    element.collectible_type,
                                    element.noOfCopies,
                                    element.user_public_address,
                                    element.is_hide == 0 ? '<span class="label label-success label-pill label-inline mr-2">Public</span>' : '<span class="label label-danger label-pill label-inline mr-2">Hidden</span>',
                                    ...
                                    element.collectibleId != null ? '<span class="label label-success label-pill label-inline mr-2">YES</span>' : '<span class="label label-danger label-pill label-inline mr-2">NO</span>',
                                    element.is_burn == 0 ? '<span class="label label-success label-pill label-inline mr-2">NO</span>' : '<span class="label label-danger label-pill label-inline mr-2">YES</span>',
                                    actions
      ...
      const collectibles = await Sequelize.query(`${query} limit ${limit} OFFSET ${skip}`, { type: QueryTypes.SELECT }).then( (collectibles) => {
            ...
            collectibles.forEach(  (element) => {
                ...
                formatted_result.push([
                                    ...
                                    element.collectible_category,
                                    element.collectible_type,
                                    element.created_wallet_address,
                                    element.owner_public_address,
                                    element.owner_balance,
                                    element.is_hide == 0 ? '<span class="label label-success label-pill label-inline mr-2">Public</span>' : '<span class="label label-danger label-pill label-inline mr-2">Hidden</span>',
                                    element.is_ban == 0 ? '<span class="label label-success label-pill label-inline mr-2">Active</span>' : '<span class="label label-danger label-pill label-inline mr-2">Banned</span>',
                                    element.curated_collectible_id != null ? '<span class="label label-success label-pill label-inline mr-2">YES</span>' : '<span class="label label-danger label-pill label-inline mr-2">NO</span>',
                                    `<div class="dropdown dropdown-inline">
								                        <a href="javascript:;" class="btn btn-sm btn-clean btn-icon" data-toggle="dropdown">
	                                        <i class="la la-cog"></i>
	                                      </a>
...
exports.editCollectible = async (req, res) => {
  ...
  const collectible = await Sequelize.query(
    "SELECT * FROM collectibles WHERE collectible_uuid = ?",
    {
  ...
  if (collectible.length == 0) {
    req.flash(
      "error",
      "Collectible not found with id = " + req.query.collectible_uuid
    );
...
exports.postEditCollectible = async(req, res, next) => {
  ...
  try {
    ...
    const update = await Collectible.update(
      collectible_info, 
      {
