...
exports.populateAllSales = async (req, res) => {
  ...
  if (typeof input.token_type != "undefined" && input.token_type) {
    logger.info(
      "[uone][controllers/reports.controller/populateAllSales Token type] " +
        input.token_type
    );
  ...
  if (
    typeof input.search_sale_by_tokenid != "undefined" &&
    input.search_sale_by_tokenid
  ) {
    logger.info(
      "[uone][controllers/reports.controller/populateAllSales Token ID] " +
        input.search_sale_by_token_address
    );
  ...
  //search by token Address
  if (
    typeof input.search_sale_by_token_address != "undefined" &&
    input.search_sale_by_token_address
  ) {
    logger.info(
      "[uone][controllers/reports.controller/populateAllSales Token Address] " +
        input.search_sale_by_token_address
    );
    query +=
      " AND td.token_address='" + input.search_sale_by_token_address + "' ";
  }

  //search with buyer wallet
  if (
    typeof input.search_sale_by_buyer != "undefined" &&
    input.search_sale_by_buyer
  ) {
    logger.info(
      "[uone][controllers/reports.controller/populateAllSales Buyer] " +
        input.search_sale_by_buyer
    );
  ...
  if (
    typeof input.search_sale_by_seller != "undefined" &&
    input.search_sale_by_seller
  ) {
    logger.info(
      "[uone][controllers/reports.controller/populateAllSales Seller] " +
        input.search_sale_by_seller
    );
  ...
  if (typeof input.from_date != "undefined" && input.from_date && typeof input.to_date != "undefined" && input.to_date) {
    logger.info(
      "[uone][controllers/reports.controller/populateAllSales From Date] " +
        input.from_date + "to date "+input.to_date
    );
  ...
  logger.info(
    "[uone][controllers/reports.controller/populateAllSales Query] " + query
  );
...
exports.exportExcel = async (req, res) => {
  ...
  if (typeof input.token_type != "undefined" && input.token_type) {
    logger.info(
      "[uone][controllers/reports.controller/exportExcel Token type] " +
        input.token_type
    );
    query += " AND c.collectible_type='" + input.token_type + "' ";
  }
  ...
  if (typeof input.from_date != "undefined" && input.from_date && typeof input.to_date != "undefined" && input.to_date) {
    logger.info(
      "[uone][controllers/reports.controller/exportExcel From Date] " +
        input.from_date + "to date "+input.to_date
    );
    query += " AND DATE(td.createdAt) >= '" + input.from_date+ "' " ;
    query += " AND DATE(td.createdAt) <='" + input.to_date + "' ";
  ...
  const sales = await Sequelize.query(`${query}`,
    ...
    worksheet.columns = [
      ...
      { header: "Collectible Name", key: "collectible_name", width: 25 },
      { header: "Collectible type", key: "collectible_type", width: 15 },
      { header: "Token ID", key: "token_id", width: 10 },
      { header: "Token Address", key: "token_address", width: 46 },
      { header: "Seller Address", key: "seller_address", width: 47 },
      { header: "Buyer Address", key: "buyer_address", width: 47 },
      { header: "Value", key: "value", width: 10 },
      { header: "Price (ETH)", key: "price", width: 20 },
    ...
    res.setHeader(
      "Content-Disposition",
      "attachment; filename=" + "UONE_SALE_REPORT_"+moment().format()+".xlsx"
    );
