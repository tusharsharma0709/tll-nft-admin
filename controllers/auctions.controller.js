...
exports.populateAllAuctions = async (req, res, next) => {
  ...
  const collectible_auctions = await Sequelize.query(`${query} limit ${limit} OFFSET ${skip}`, { type: QueryTypes.SELECT }).then(collectible_auctions => {
    ...
    collectible_auctions.forEach(element => {
      ...
      formatted_result.push([serial_number += 1, element.collectible_name, element.token_type, element.seller_address, element.quantity, element.currency_symbol, element.currency_address, web3.utils.fromWei(starting_price_bn, 'ether'), web3.utils.fromWei(reserved_price_bn, 'ether'), moment.utc(element.auction_start).format("DD/MM/YYYY hh:mm:ss"), moment.utc(element.auction_end).format("DD/MM/YYYY hh:mm:ss"), element.is_featured == 0 ? '<span class="label label-danger label-pill label-inline mr-2">NO</span>' : '<span class="label label-success label-pill label-inline mr-2">YES</span>', status, actions]);
    });
...
exports.populateBidsOfAuction = async (req, res, next) => {
  ...
  const collectible_auction_bids = await Sequelize.query(`${query} limit ${limit} OFFSET ${skip}`, { type: QueryTypes.SELECT }).then(collectible_auction_bids => {
    ...
    collectible_bids.forEach(element => {
      ...
      //shop.productImagesTemp  = shop.productImages;
      formatted_result.push([serial_number += 1, element.bidder_address, web3.utils.fromWei(price_bn, 'ether'), moment.utc(element.createdAt).format("DD/MM/YYYY hh:mm:ss"), status]);
    });
    //console.log(collectibles.length);
    res.json({
      "draw": input.draw,
