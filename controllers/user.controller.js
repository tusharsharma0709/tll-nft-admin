...
exports.populateAllUsers = async (req, res) => {
  ...
  const users = await Sequelize.query(`${query} limit ${limit} OFFSET ${skip}`, { type: QueryTypes.SELECT }).then(users => {
    ...
    users.forEach(element => {
      ...
      formatted_result.push([serial_number += 1, element.username, element.user_public_address, element.email, element.is_ban == 0 ? '<span class="label label-success label-pill label-inline mr-2">Active</span>' : '<span class="label label-danger label-pill label-inline mr-2">Banned</span>', element.curated_artist_id != null ? '<span class="label label-success label-pill label-inline mr-2">YES</span>' : '<span class="label label-danger label-pill label-inline mr-2">NO</span>', `<div class="dropdown dropdown-inline">
  ...
  if (req.body.cmd == "make_curated_artist") {
    ...
    if (curated_user_exist.length == 0) {
      ...
      await db.sequelize.query(sql, {
        replacements: [req.body.id, req.body.user_public_address],
...
exports.UserListExportExcel = async (req, res) => {
  ...
  const users = await Sequelize.query(`${query}`, { type: QueryTypes.SELECT }).then(users => {
    users = JSON.parse(JSON.stringify(users));
    let recordsFiltered = users.length;
    console.log("filtered records");
    console.log(recordsFiltered);
    ...
    worksheet.columns = [{ header: "Sno", key: "sno", width: 5 }, { header: "Username", key: "user_name", width: 46 }, { header: "Wallet Address", key: "user_wallet_address", width: 46 }, { header: "Email", key: "email", width: 46 },
    //{ header: "User Status", key: "user_status", width: 15 },
    { header: "Is KYC Verified?", key: "is_kyc_verified", width: 15 }, { header: "KYC Appproved At", key: "kyc_approval_at", width: 20 }, { header: "Is Mint Allowed", key: "is_mint_allow", width: 15 }];
    ...
    res.setHeader("Content-Disposition", "attachment; filename=" + "BAS_USER_REPORT_" + moment().format() + ".xlsx");
